...
HRESULT CGameInstance::Initialize_Engine(HINSTANCE hInst, _uint iNumLevels, const GRAPHIC_DESC& GraphicDesc, ID3D11Device** ppDevice, ID3D11DeviceContext** ppContext)
{
	...
	/* 그래픽 디바이스 초기화. */
	if (FAILED(m_pGraphic_Device->Ready_Graphic_Device(GraphicDesc.hWnd, GraphicDesc.eWindowMode, GraphicDesc.iViewportSizeX, GraphicDesc.iViewportSizeY, ppDevice, ppContext)))
		return E_FAIL;
	...
	if (FAILED(m_pImgui_Manager->Ready_Imgui(GraphicDesc.hWnd, m_pDevice, m_pContext)))
		return E_FAIL;

	if (FAILED(m_pComponent_Manager->Add_Prototype(m_iStaticLevelIndex, m_pPrototypeTransformTag, CTransform::Create(*(ppDevice), *(ppContext)))))
		return E_FAIL;	
...
void CGameInstance::Clear_Level(_uint iLevelIndex)
{
	...
	m_pComponent_Manager->Clear_Component(iLevelIndex);
}
...
HRESULT CGameInstance::Clone_GameObject(_uint iLevelIndex, const _tchar * pLayerTag, const _tchar * pPrototypeTag, const _tchar* pMyName, void * pArg)
{
 	if (nullptr == m_pObject_Manager)
		return E_FAIL;

	return m_pObject_Manager->Clone_GameObject(iLevelIndex, pLayerTag, pPrototypeTag, pMyName, pArg);
}
...
//void CGameInstance::Play_Sound(const _tchar * pSoundKey, _float fVolume, _bool bIsBGM, _int iManaulChannelIndex)
//{
//	if (nullptr == m_pSound_Manager)
//		return;
//
//	return m_pSound_Manager->Play_Sound(pSoundKey, fVolume, bIsBGM, iManaulChannelIndex);
//}
...
//		return;
//
//	return Set_Volume(iManualChannelIndex, fVolume);
//}
//
...
//		return;
//
//	return Set_MasterVolume(fVolume);
//}
...
/*void CGameInstance::Set_SoundDesc(const _tchar * pSoundKey, CSound::SOUND_DESC & SoundDesc)
{
	...
	return Set_SoundDesc(pSoundKey, SoundDesc);
}*/
