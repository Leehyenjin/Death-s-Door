...
HRESULT CNavigation::Init_ProtoClone(const _tchar * pNavigationDataFilePath)
{
	/*2. Navigation에 함수 하나 만들어 Initialize_Proto랑 똑같은 역할 하는 함수 하나 만들어 그거 여기서 돌려
	->저장된 포인트 불러오기, 불러온 포인트 순서대로 3개씩 Cell에 던져서 생성, 이웃 연결*/
...
_int CNavigation::isPacking_NaviCell(HWND hWnd, _uint iWinsizeX, _uint iWinsizeY)
{
	POINT	ptMouse{};
	...
	_matrix		ProjMatrixnv;
	ProjMatrixnv = ENGINE->Get_TransformMatrix(CPipeLine::D3DTS_PROJ);
	ProjMatrixnv = XMMatrixInverse(nullptr, ProjMatrixnv);
	vecMousePos = XMVector3TransformCoord(vecMousePos, ProjMatrixnv);
	...
	memcpy(&vRayPos, &ViewMatrixInv.r[3], sizeof(_vector));
	vecMousePos = XMVector3TransformCoord(vecMousePos, ViewMatrixInv);
	vRayDir = XMVector3Normalize(vecMousePos - vRayPos);
...
_float CNavigation::Height_OnNavigation(_fvector vPos)
{
	_vector			Plane;
...
CComponent * CNavigation::Clone(void * pArg)
{
	...
	{
		MSG_BOX("Failled to Clone : CNavigation");
		Safe_Release(pInstance);
	}
