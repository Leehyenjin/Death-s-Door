...
CAnimation * CModel::Get_AnimIndex(_uint AnimIndex)
{
...
void CModel::Set_Animation(_uint iAnimation)
{
	// 0이거나 0보다 크면 애니메이션을 재생한다 Set_AnimIndex가 아닌 Set_Animation을 함수를 쓰면됨
	if (0 > iAnimation || m_iNumAnimations <= iAnimation)
		return;

	//m_bLerpReset = true;
	m_iNextAnimation = iAnimation;
	m_bChanged = true;
}
