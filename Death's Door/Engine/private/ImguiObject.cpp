...
void CImguiObject::Control_Component(const char * pComponentName)
{
	...
	if (!strcmp(pComponentName, "Com_Texture"))
	{
		if (ImGui::CollapsingHeader("Com_Texture", ImGuiTreeNodeFlags_DefaultOpen))
		{
		}
	}

	if (!strcmp(pComponentName, "Com_Shader"))
	{
		if (ImGui::CollapsingHeader("Com_Shader", ImGuiTreeNodeFlags_DefaultOpen))
		{
		}
	}

	if (!strcmp(pComponentName, "Com_VIBuffer"))
	{
		if (ImGui::CollapsingHeader("Com_VIBuffer", ImGuiTreeNodeFlags_DefaultOpen))
		{

		}
	}

	if (!strcmp(pComponentName, "Com_Renderer"))
	{
		if (ImGui::CollapsingHeader("Com_Renderer", ImGuiTreeNodeFlags_DefaultOpen))
		{
	...
	if (ENGINE->Mouse_Down(DIM_LB))
	{
		for (auto Layer : *m_mapLayer)
		{
			for (auto Object : *Layer.second->Get_ObjectList())
			{
				...
				_bool isPeek = static_cast<CCalculator*>(Object->Get_Component(TEXT("Com_Calculator")))
					->Ray_CubePeeking(
...
void CImguiObject::SaveLoad_Object()
{
	...
	/*ImGui::SetWindowPos(ImVec2(100.f, 100.f)); 위치 고정하는 기능(x,y)*/
...
void CImguiObject::CameraChange()
{
	ImGui::Begin("CameraChange");
	void BeginFrame();
