...
// 모델 중, 현재 그릴려고 하는 메시에 뼈를 받아온다(뼈의 행렬)
...
struct VS_IN
{
	...
	// 현재 정점에게 곱해져야할 뼈들(최대4개)의 행렬을 만든다
...
VS_OUT VS_MAIN(VS_IN In)
{
	...
	// 여기서 fWeightW를 만들어 주고 사용하는 이유가 뼈가 없는 메시도있어서 그 메시들이 나타나지않아서
	// 강제로 1이란 항등행렬 값을 넣어줘서 그려지게끔 만들어 준 것 이다
	matrix		BoneMatrix = g_BoneMatrices[In.vBlendIndex.x] * In.vBlendWeight.x +
		g_BoneMatrices[In.vBlendIndex.y] * In.vBlendWeight.y +
		g_BoneMatrices[In.vBlendIndex.z] * In.vBlendWeight.z +
		g_BoneMatrices[In.vBlendIndex.w] * fWeightW;

	// Position과 Normal을 새롭게 곱해서 만들어 준다 로컬스페이스의 위치를 곱해서 뼈의 상태를 나타내 주겠다는 의미
